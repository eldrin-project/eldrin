...
>
> **Slice 3 (AI enhancement via eldrin-workflows) done + live-validated 2026-07-03** — company auto-enrichment (10.5) and the AI extraction/apply loop shipped. Ghost-activity detection (10.7) + ghost dashboard widget (10.10) + deal auto-detection remain, split into a follow-up mini-slice (Slice 3b). Spec/plan: `docs/superpowers/{specs,plans}/2026-07-03-ai-enhancement-workflows*`; SDD ledger `.superpowers/sdd/progress.md`.
...
- [x] Step 10.5: Implement company auto-enrichment — DONE (Slice 3, 2026-07-03, via eldrin-workflows, not an in-CRM enrichment service). Provisional companies are auto-created from non-freemail sender domains (`isEnrichableDomain` SSRF/noise guard), a `company.created` event fires the "CRM: Enrich new company" workflow (fetch homepage → `html_extract` title/meta/OG → `POST /api/enhancement/companies/:id` apply, fill-empty-only), and an AI email-extraction workflow fills contact job title/phones/social. External paid providers remain a config seam (default = free homepage-metadata + mock LLM). Live-validated end-to-end.

...
- [ ] **DECISION NEEDED — app→core emission auth:** core's auth middleware 401s `POST /api/events/emit` because the SDK event client sends only `X-Eldrin-App-Id` (no JWT). CRM's `contact.created` etc. cannot be emitted until this is resolved. Options: shared app secret header, core-minted service tokens, or allowlisting the app-scoped event endpoints (auth stays header-based in the route handlers). Touches eldrin-core auth — deliberately not changed autonomously.

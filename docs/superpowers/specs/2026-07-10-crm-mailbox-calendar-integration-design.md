...
- **D5 — Schedule meeting is a synchronous proxy write, not a bus command.** Interactive scheduling needs immediate success/failure and the created event id; the event bus stays the mirror-back channel only.
...
- **D10 — User-owned fields survive mirror updates.** Meeting activities gain `agenda` and `outcome` (user-editable, stored in CRM only). Mirror updates overwrite ONLY calendar-sourced fields (title/times/location/attendees/metadata), never agenda/outcome or other user edits. Implemented as an explicit field whitelist in the mirror update path.
- **D11 — Richer timeline rendering.** Timeline entries for `type-meeting` render start–end time, location, attendee chips (with matched-contact links), and an "Open in Calendar" link to the Calendar app root (`/eldrin-calendar`). No event deep-link — the calendar router has no event/date query param today and adding one is out of scope.
...
- **CRM unit:** migration/schema; capture auto-assign (new record, fill-empty backfill, no-overwrite); schedule endpoint (happy path, each D7 fallback, validation errors, proxy failure — calendar proxied via mocked fetch); mirror deal-linking; mirror whitelist (agenda/outcome preserved); widget annotation mapping.

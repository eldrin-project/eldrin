 ...
 *
 * The manifest declares "entry": "/eldrin-factorial.js" which only exists
...
function devShellCompat(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      if (id.endsWith('/src/eldrin-factorial.tsx')) {
        return code.replace(/import\s+['"]\.\/index\.css['"];?\n?/, '');
      }
    },
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (req.url === '/eldrin-factorial.js') {
          req.url = '/src/eldrin-factorial.tsx';
          return next();
        }
        if (req.url === '/eldrin-factorial.css') {
          try {
...
export default defineConfig({
  ...
  build: {
    ...
    minify: true,
    lib: {
      entry: './src/eldrin-factorial.tsx',
      name: 'eldrinFactorial',
      formats: ['es'],
      fileName: 'eldrin-factorial',
    },
  },
});
